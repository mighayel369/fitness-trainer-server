...
export class BookingController {
  constructor(
    ...
    @inject("AcceptRescheduleBookingRequest") private _acceptRescheduleRequestUseCase: IProcessTrainerRescheduleUseCase,
    @inject("DeclineRescheduleBookingRequest") private _declineRescheduleRequestUseCase: IProcessTrainerRescheduleUseCase,
  ...
  checkoutAndBook = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.log(input)
      const bookingSummary:BookingResponseDTO=await this._finalizeBookingUseCase.bookSessionWithTrainer(input);
...
acceptReschedule = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await this._acceptRescheduleRequestUseCase.execute({ bookingId});
  ...
  declineReschedule = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await this._declineRescheduleRequestUseCase.execute(input);
